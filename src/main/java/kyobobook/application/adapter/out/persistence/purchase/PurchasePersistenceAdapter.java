...
public class PurchasePersistenceAdapter implements PurchasePersistencePort {
    ...
    @Override
    public List<PurchaseProduct> getPurchasedInCart(Integer limitYear) throws Exception {
        
        List<PurchaseProductEntity> queryResult = this.reader.selectPurchaseProducts(limitYear);
        List<PurchaseProduct> result = new ArrayList<PurchaseProduct>();
        queryResult.stream().forEach(purchaseProduct -> {
            result.add(PurchaseProduct.builder().
                    ordrDate(purchaseProduct.getOrdrDate()).
                    unfyCmdtId(purchaseProduct.getUnfyCmdtId()).
                    OnlnCmdtDvsnCode(purchaseProduct.getOnlnCmdtDvsnCode()).
                    orderQty(purchaseProduct.getOrderQty()).
                    build());
        });
